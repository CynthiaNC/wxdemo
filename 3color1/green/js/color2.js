!function() {
    ...
    e = {
        ...
        render: function(e, f) {
            ...
            var h1 = Math.floor(Math.random() * e * e),
            h2 = Math.floor(Math.random() * e * e),
                i = this.getColor(255 - this.d),  //干扰色
                j = this.getLvColor(i[0]);        //目标颜色
            while(h1 == h2)
                h2 = Math.floor(Math.random() * e * e);
            box.find(span).css("background-color", i[1]);
            box.find(span).eq(h1).css("background-color", j[1]).data("type", "a");
            box.find(span).eq(h2).css("background-color", j[1]).data("type", "a");
